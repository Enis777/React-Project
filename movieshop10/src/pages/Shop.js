...
function Shop() {
    ...
    useEffect(() => {
        ...
        axios.get(proxyUrl + `https://api.themoviedb.org/3/discover/movie?api_key=3e52e2f5350ae60de5e2fc58e818d2a0&page=${page}`)
        .then(resp => {
            if(resp.status === 200) {
    ...
    const handleSearch = e => {
      ...
      switch(e.keyCode) {
        case 13: 
        const proxyUrl = 'https://cors-anywhere.herokuapp.com/';
          axios.get(proxyUrl + `https://api.themoviedb.org/3/search/movie?api_key=3e52e2f5350ae60de5e2fc58e818d2a0&query=${query}`)
          .then(resp => {
              if(resp.status === 200) {
                setMovies(resp.data.results)
                setTotalPages(resp.data.total_pages)
